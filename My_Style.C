...
TString extraText5   = "Work in Progress"; // For Simulation Preliminary on two lines
	  ...
	  if( writeExtraText ) 
	    {
	      ...
	      if (extraText5!="") // For Simulation Preliminary
		latex.DrawLatex(posX_, posY_-relExtraDY*cmsTextSize*t
				- relExtraDY*extraTextSize*t, extraText5);
...
TCanvas* tdrCanvas(const char* canvName, TH1D *h,
  ...
  float L = (square ? 0.125*W_ref : 0.13*W_ref);
  float R = (square ? 0.125*W_ref : 0.135*W_ref);
  ...
  //assert(h);
  //h->GetYaxis()->SetTitleOffset(square ? 1. : 0.85);
